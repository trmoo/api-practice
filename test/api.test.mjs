...
export function run(T) {
  ...
  T.group('★ 교실 보호장치 — 같은 요청을 되풀이해도 서버를 두드리지 않는가');
  api.기억비우기();
  T.ok('비우면 기억이 0개', api.기억개수(), 0);
  ...
  return Promise.all([
    ...
    // ⚠ 연습 모드는 서버를 부르지 않으므로 캐시에 넣지 않는다.
    //   캐시는 「실제 호출」에서만 쓰인다. 연습 모드 결과를 캐시하면
    //   모드를 바꿔도 옛 답이 나오는 사고가 난다.
    T.ok('연습 모드 결과는 기억에 담지 않는다', api.기억개수(), 0);
    T.falsy('연습 모드 결과에 기억 표시가 없다', 찾음.기억에서꺼냄);
  });
}
